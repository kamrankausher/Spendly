from flask import Flask, render_template, request, redirect, session, url_for, flash, Response, jsonify, g
...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "spendly_v7_dev_fallback_key_change_in_prod")
...
# Register DB teardown — auto-closes connection at end of every request
...
def add_cache_headers(response):
    """Prevent browser from caching HTML pages — fixes stale session issues."""
...
def register():
    ...
    if request.method == "POST":
        ...
        cur = conn.execute("INSERT INTO users (name,email,password_hash,country,gender) VALUES (?,?,?,?,?)",
                           (name, email, h, country, gender))
        uid = cur.lastrowid
...
def dashboard():
    ...
    summary = conn.execute(
        ...
        (uid, som, eom)).fetchone()
    total_spent = summary['total']

    expenses = [dict(r) for r in conn.execute(
        "SELECT * FROM expenses WHERE user_id=? AND date>=? AND date<=? ORDER BY date DESC LIMIT 5",
        (uid, som, eom)).fetchall()]

    subs = [dict(r) for r in conn.execute(
        "SELECT * FROM subscriptions WHERE user_id=? ORDER BY is_active DESC LIMIT 3", (uid,)).fetchall()]
    goals = [dict(r) for r in conn.execute(
        "SELECT * FROM goals WHERE user_id=? LIMIT 3", (uid,)).fetchall()]

    cats_query = conn.execute(
...
def profile():
    ...
    if request.method == "POST":
        ...
        conn.execute("UPDATE users SET name=?,phone=?,age=?,gender=?,country=?,currency=?,language=? WHERE id=?",
                     (f.get("name"), f.get("phone"), int(f.get("age") or 0), f.get("gender"),
                      f.get("country"), f.get("currency"), f.get("language"), session['user_id']))
        ...
        conn.commit()
        # Clear cached user so next context_processor fetch gets fresh data
...
def reports():
    ...
    cats = [dict(r) for r in conn.execute(
        "SELECT category,SUM(amount) as total,COUNT(*) as cnt FROM expenses WHERE user_id=? GROUP BY category ORDER BY total DESC",
        (uid,)).fetchall()]
    expenses = [dict(r) for r in conn.execute(
        "SELECT * FROM expenses WHERE user_id=? ORDER BY date DESC LIMIT 50", (uid,)).fetchall()]
    subs = [dict(r) for r in conn.execute(
        "SELECT * FROM subscriptions WHERE user_id=?", (uid,)).fetchall()]
    budget_row = conn.execute("SELECT amount FROM budgets WHERE user_id=?", (uid,)).fetchone()
    budget = budget_row['amount'] if budget_row else 0
    ...
    monthly_raw = conn.execute(
        ...
        (uid,)).fetchall()
    monthly_trends = [dict(r) for r in monthly_raw]
    monthly_trends.reverse()
    return render_template("reports.html", categories=cats, expenses=expenses, subscriptions=subs,
                           budget=budget, total_spent=total, monthly_trends=monthly_trends)

@app.route("/export")
@login_required
def export_csv():
    conn = get_db()
    rows = conn.execute("SELECT date,category,amount,description FROM expenses WHERE user_id=? ORDER BY date DESC",
                        (session['user_id'],)).fetchall()
    si = io.StringIO()
    w = csv.writer(si)
    w.writerow(['Date', 'Category', 'Amount', 'Description'])
    for r in rows:
        w.writerow([r['date'], r['category'], r['amount'], r['description']])
    return Response(si.getvalue(), mimetype="text/csv",
                    headers={"Content-disposition": "attachment; filename=spendly_report.csv"})
...
def alerts():
    conn = get_db()
    subs = [dict(r) for r in conn.execute(
        "SELECT * FROM subscriptions WHERE user_id=? ORDER BY is_active DESC, end_date ASC",
        (session['user_id'],)).fetchall()]
...
def add_subscription():
    ...
    conn = get_db()
    conn.execute(
        "INSERT INTO subscriptions (user_id,platform,cost,start_date,end_date,duration_months,is_active) VALUES (?,?,?,?,?,?,1)",
        (session['user_id'], f.get("platform"), float(f.get("cost", 0)),
         f.get("start_date"), f.get("end_date"), int(f.get("duration", 1))))
    conn.commit()
...
def toggle_sub(sid):
    conn = get_db()
    row = conn.execute("SELECT is_active FROM subscriptions WHERE id=? AND user_id=?",
                       (sid, session['user_id'])).fetchone()
    if row:
        conn.execute("UPDATE subscriptions SET is_active=? WHERE id=?",
                     (0 if row['is_active'] else 1, sid))
        conn.commit()
    return redirect(url_for('alerts'))
...
def tracking():
    ...
    if filt == 'week':
        ...
        sd = date(today.year, 1, 1).isoformat()
    else:
        sd = "2000-01-01"

    expenses = [dict(r) for r in conn.execute(
        "SELECT * FROM expenses WHERE user_id=? AND date>=? ORDER BY date DESC", (uid, sd)).fetchall()]
    subs = [dict(r) for r in conn.execute(
        "SELECT * FROM subscriptions WHERE user_id=?", (uid,)).fetchall()]
    ...
    for cat in ["Entertainment", "Shopping", "Food", "Other"]:
        if cat in cats and total > 0:
            ...
            if pct > 15:
                suggestions.append({
                    "category": cat, "amount": cats[cat], "pct": round(pct, 1),
    ...
    if not suggestions and total > 0:
        suggestions.append({
            "category": "General", "amount": total, "pct": 100,
            "tip": "Great job! Your spending looks balanced. 🎉", "severity": "low"
        })
    return render_template("tracking.html", expenses=expenses, subscriptions=subs,
                           suggestions=suggestions, cat_data=cats, current_filter=filt)
...
def add_expense():
    ...
    conn.execute("INSERT INTO expenses (user_id,amount,category,date,description) VALUES (?,?,?,?,?)",
                 (session['user_id'], float(f.get("amount", 0)), f.get("category"),
...
def edit_expense(eid):
    ...
    conn.execute("UPDATE expenses SET amount=?,category=?,date=?,description=? WHERE id=? AND user_id=?",
                 (float(f.get("amount", 0)), f.get("category"), f.get("date"),
                  f.get("description", ""), eid, session['user_id']))
    conn.commit()
...
def update_budget():
    conn = get_db()
    conn.execute("UPDATE budgets SET amount=? WHERE user_id=?",
                 (float(request.form.get("amount", 0)), session['user_id']))
    conn.commit()
...
def goals_page():
    conn = get_db()
    goals = [dict(r) for r in conn.execute(
        "SELECT * FROM goals WHERE user_id=? ORDER BY deadline ASC", (session['user_id'],)).fetchall()]
...
def add_goal():
    ...
    conn.execute("INSERT INTO goals (user_id,title,target_amount,saved_amount,deadline) VALUES (?,?,?,?,?)",
                 (session['user_id'], f.get("title"), float(f.get("target", 0)),
                  float(f.get("saved", 0)), f.get("deadline")))
    conn.commit()
...
def update_goal(gid):
    conn = get_db()
    conn.execute("UPDATE goals SET saved_amount=? WHERE id=? AND user_id=?",
                 (float(request.form.get("saved", 0)), gid, session['user_id']))
    conn.commit()
...
def analytics():
    ...
    monthly = [dict(r) for r in conn.execute(
        "SELECT strftime('%Y-%m', date) as month, SUM(amount) as total FROM expenses WHERE user_id=? GROUP BY month ORDER BY month DESC LIMIT 12",
        (uid,)).fetchall()]
    monthly.reverse()
    ...
    if budget > 0:
        ...
        if spend_ratio > 1:
            score = max(10, int(100 - spend_ratio * 50))
        elif spend_ratio > 0.8:
            score = int(100 - (spend_ratio - 0.8) * 200)
        else:
            score = min(100, int(100 - (spend_ratio * 20)))
    ...
    if budget > 0:
        ...
        if pct_used > 90:
            insights.append({"icon": "🚨",
                             "text": f"You've used <strong>{pct_used:.0f}%</strong> of your budget. Consider pausing non-essential spending."})
        elif pct_used > 70:
            insights.append({"icon": "⚡",
                             "text": f"<strong>{pct_used:.0f}%</strong> of budget used. You're on track but be mindful of big purchases."})
        else:
            insights.append({"icon": "🏆",
                             "text": f"Excellent! Only <strong>{pct_used:.0f}%</strong> used. You're well within your budget this month."})
    if top_cat['total'] > 0:
        insights.append({"icon": "📊",
                         "text": f"Your highest spending category is <strong>{top_cat['category']}</strong> at ₹{top_cat['total']:,.0f}."})
    if avg_daily > 0:
        monthly_proj = avg_daily * 30
        insights.append({"icon": "📈",
                         "text": f"At your current rate of ₹{avg_daily:,.0f}/day, you'll spend ~₹{monthly_proj:,.0f} by month end."})
